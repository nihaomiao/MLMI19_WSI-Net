# Testing those models using classification branch
# Author: Haomiao Ni
# Created on 18/11/02
...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes, track_running_stats=True,
                 momentum=0.1, thre=0.3):
        self.thre = thre
        self.inplanes = 64
        ...
        self.AuxSoftmax = torch.nn.Softmax(dim=1)
    ...
    def forward(self, x):
        # start = time.time()
        new_x = torch.zeros((x.size()[0], x.size()[1], x.size()[2] / 8 + 1,
                             x.size()[3] / 8 + 1))
        new_x[:, 0, :, :] = 10
        new_x = new_x.cuda()
        x = self.conv1(x)
        ...
        aux_pred = self.AuxSoftmax(aux)
        x_ind = aux_pred[:, 1]>self.thre
        #x_ind = torch.randint(0, 2, aux_pred[:, 1].size()).long().cuda()
        x = x[x_ind, :, :, :]
        if x.nelement()==0:
            return new_x, x_ind, aux_pred
        x = self.layer3(x)
        x = self.layer4(x)
        x = self.layer5(x)
        new_x[x_ind, :, :, :] = x
...
def Res_Deeplab(num_classes=21, is_refine=False, track_running_stats=True, momentum=0.1, thre=0.3):
    if is_refine:
        model = ResNet_Refine(Bottleneck, [3, 4, 23, 3], num_classes)
    else:
        model = ResNet(Bottleneck, [3, 4, 23, 3], num_classes, track_running_stats, momentum, thre=thre)
    return model
