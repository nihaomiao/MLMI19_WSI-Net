# Jointly training the classification branch and DeepLab
# Author: Haomiao Ni
# Created on 18/10/26
...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes, train_aux=True, train_joint=True, track_running_stats=True,
                 momentum=0.1):
        ...
        if not train_aux or train_joint:
            self.layer3 = self._make_layer(block, 256, layers[2], stride=1, dilation=2,
                                           track_running_stats=track_running_stats, momentum=momentum)
            self.layer4 = self._make_layer(block, 512, layers[3], stride=1, dilation=4,
                                           track_running_stats=track_running_stats, momentum=momentum)
...
class InceptionAux(nn.Module):
    def __init__(self, in_channels, num_classes):
        ...
        # self.avgpool4 = nn.AdaptiveAvgPool2d((16, 16))
...
def Res_Deeplab(num_classes=21, is_refine=False, track_running_stats=True, momentum=0.1,
                train_aux=True, train_joint=True):
    if is_refine:
        model = ResNet_Refine(Bottleneck, [3, 4, 23, 3], num_classes)
    else:
        model = ResNet(Bottleneck, [3, 4, 23, 3], num_classes, train_aux, train_joint, track_running_stats, momentum)
    return model
