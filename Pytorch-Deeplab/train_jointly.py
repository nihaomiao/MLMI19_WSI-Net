# Jointly training deeplab and classification branch
# Note that size is fixed during training
# Author: Haomiao Ni
# Created on 10/26/18
...
def get_arguments():
    ...
    parser.add_argument("--img-dir", type=str, default='/disk8t-1/deeplab-xiangya2-IC/imgshots-FrozenBN-IC-joint',
                        help="Where to save images of the model.")
    parser.add_argument("--cls-loss-coeff", default=2.3)
    parser.add_argument("--dlb-loss-coeff", default=2.3)
...
def main():
    ...
    while actual_step < args.final_step:
        ...
        for i_iter, batch in enumerate(trainloader):
            ...
            dlb_pix_loss, dlb_pat_loss = dlb_loss_calc(pred, labels)
            loss = args.cls_loss_coeff * cls_loss.double() + \
                   dlb_pix_loss.double() + args.dlb_loss_coeff * dlb_pat_loss
            losses.update(loss.item(), pred.size(0))
            cls_losses.update(cls_loss.item(), aux_pred.size(0))
            dlb_pix_losses.update(dlb_pix_loss.item(), pred.size(0))
            dlb_pat_losses.update(dlb_pat_loss.item(), pred.size(0))
            ...
            if actual_step % args.print_freq == 0:
                print('iter: [{0}]{1}/{2}\t'
                      ...
                      'DLB_Patch Loss {dlb_patch_loss.val:.4f} ({dlb_patch_loss.avg:.4f})\t'
                    .format(
                    ...
                    dlb_patch_loss=dlb_pat_losses, cls_accuracy=cls_accuracy,
